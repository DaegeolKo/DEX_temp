...
class ClampedParamMap(torch.nn.Module):
    """Wrap param_map and clamp the scale part to avoid exploding/vanishing scales."""
    ...
    def forward(self, x):
        out = self.base(x)
        # even indices: shift, odd indices: scale
        shift = out[:, 0::2, ...]
...
def train(args):
    ...
    model = build_model(
        dim=args.dim, num_layers=args.num_layers, hidden=args.hidden, scale_map=args.scale_map
    ).to(device)
    if args.compile:
        try:
...
def parse_args():
    p = argparse.ArgumentParser(description="Train RealNVP on 12D vectors (.pt/.npy/.npz/.csv/.txt).")
    p.add_argument(
        "--data-path",
        type=str,
