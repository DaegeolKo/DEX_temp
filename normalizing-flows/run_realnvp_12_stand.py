...
class ClampedParamMap(torch.nn.Module):
    """AffineCoupling용 scale 로그를 클램프해 NaN/폭발을 완화."""

    def __init__(self, base, clamp: float):
        super().__init__()
        self.base = base
        self.clamp = clamp
    ...
    def forward(self, x):
        out = self.base(x)
        # 짝수 인덱스는 shift, 홀수 인덱스는 scale
        shift = out[:, 0::2, ...]
        scale = out[:, 1::2, ...].clamp(-self.clamp, self.clamp)
        out[:, 0::2, ...] = shift
        out[:, 1::2, ...] = scale
        return out


def build_model(dim: int, num_layers: int, hidden: int, scale_map: str):
    base = nf.distributions.base.DiagGaussian(dim)
    flows = []
    split = dim // 2
    out_dim = 2 * (dim - split)  # shift + scale
    for _ in range(num_layers):
        param_map = nf.nets.MLP([split, hidden, hidden, out_dim], init_zeros=True)
        if build_model.scale_clamp > 0:
            param_map = ClampedParamMap(param_map, build_model.scale_clamp)
        flows.append(
            nf.flows.AffineCouplingBlock(
                param_map, scale=True, scale_map=scale_map, split_mode="channel"
            )
        )
        flows.append(nf.flows.Permute(dim, mode="swap"))
    return nf.NormalizingFlow(base, flows)
build_model.scale_clamp = 0.0
...
def train(args):
    ...
    x_std, mean, std = standardize(x_raw, eps=args.std_eps)
    ...
    build_model.scale_clamp = float(args.scale_clamp)
    model = build_model(dim=args.dim, num_layers=args.num_layers, hidden=args.hidden, scale_map=args.scale_map).to(device)
    if args.compile:
        try:
    ...
    def maybe_save(tag=None):
        ...
        torch.save(
            {"model": model.state_dict(), "args": vars(args), "mean": mean.cpu(), "std": std.cpu()},
            out_path,
        )
...
def parse_args():
    p = argparse.ArgumentParser(description="Train RealNVP on standardized 12D vectors.")
    ...
    p.add_argument("--scale-map", type=str, default="sigmoid", choices=["exp", "sigmoid", "sigmoid_inv"])
    p.add_argument("--scale-clamp", type=float, default=0.0, help="scale logits clamp (<=0이면 비활성화)")
