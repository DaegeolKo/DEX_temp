...
class Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Go2FlatEnvCfg | Go2RoughEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._episode_sums = {
            ...
            for key in [
                ...
                "stop_penalty_lin", "stop_penalty_ang", "ik_failure_penalty"
            ]
        }
        ...
        # function에 통합용
        self._ik_failed = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        # nan_mask = torch.isnan(target_joint_angles)
        
        # if nan_mask.any():
        #     # 문제가 발생한 env 인덱스 찾기
        #     problem_env_indices = nan_mask.any(dim=1).nonzero(as_tuple=False).squeeze()
            
        #     self.ik_failure_penalty[problem_env_indices] = -10.0  # 페널티 값
        #     self.reset_buf[problem_env_indices] = 1              # 리셋 신호
            
        #     # self._processed_actions 에는 이전 스텝의 유효한 값이 들어있다고 가정
        ...
        if nan_mask.any():
            problem_env_indices = nan_mask.any(dim=1).nonzero(as_tuple=False).view(-1)
            
            # 1. 여기에 IK가 실패했다고 플래그만 남김
            self._ik_failed[problem_env_indices] = True
            
            # 2. 에피소드 종료 신호는 여기서 보내야 함 (매우 중요)
            # reset_buf는 step 함수의 핵심 로직에 바로 사용되므로,
            # 보상과 분리하여 즉시 처리하는 것이 안전합니다.
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # IK 발산 샘플링 시
        ik_failure_penalty = torch.where(self._ik_failed, -10.0, 0.0)
        ...
        rewards = {
            ...
            "ik_failure_penalty": ik_failure_penalty
        }
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        #
        default_joint_pos_idx = self._robot.data.default_joint_pos[env_ids]
        self._processed_actions[env_ids] = default_joint_pos_idx
        #
